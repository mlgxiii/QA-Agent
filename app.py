...
_TIMEOUT = 600  # 10-minute hard cap
_MAX_LOG_LINES = 500
...
def run_discovery(
    ...
    cmd = [
        sys.executable, "-m", "saas_agent.cli",
        "--output", report_path,
        "--model", model,
    ]
    ...
    try:
        ...
        if timed_out:
            log += f"\n⏱️  Timed out after {_TIMEOUT // 60} minutes.\n"
            yield log, ""
            return
...
with gr.Blocks(title="SaaS Discovery Agent") as demo:
    ...
    custom_query_input = gr.Textbox(
        label="Custom GitHub search query (optional — add your own on top of the checkboxes)",
